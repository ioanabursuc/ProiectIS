...
def register():
    ...
    try:
        cursor.execute(
            "INSERT INTO users (username, email, password, role, sex, varsta, greutate, inaltime) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
            (username, email, password, role, sex, varsta, greutate, inaltime)
        )
        conn.commit()
...
def schimba_abonament():
    ...
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE users SET membership = %s WHERE id = %s", (abonament_nou, session['user_id']))
    conn.commit()

    # Obține detaliile actualizate ale utilizatorului
    cursor.execute("SELECT membership FROM users WHERE id = %s", (session['user_id'],))
    user_data = cursor.fetchone()
    cursor.close()
    conn.close()
